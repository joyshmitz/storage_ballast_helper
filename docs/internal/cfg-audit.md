...
| `src/scanner/walker.rs` | 608 | Returns an empty open-file set on every non-Linux target. | `PAL-method` | `open_file_keys()` | Current macOS behavior can miss open-file vetoes. Replace with a platform result that can report unsupported/incomplete states. |
...
| `src/scanner/walker.rs` | 692 | Returns an empty, complete ancestor set on non-Linux. | `PAL-method` | `open_path_ancestors(root_paths)` | Reporting `complete = true` is misleading on macOS because no scan happened. |
