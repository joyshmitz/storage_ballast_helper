...
| `src/cli_app.rs` | 954 | Rejects explicit `--systemd` only when the detected service kind is not systemd. | `PAL-method` | `service_manager_kind()` | Closed for install: `run_install()` calls `detect_platform()`, `resolve_install_service()` defaults omitted flags to the detected service kind, and `install_auto_selects_*` plus wrong-service tests cover both branches. |
| `src/cli_app.rs` | 960 | Rejects explicit `--launchd` only when the detected service kind is not launchd. | `PAL-method` | `service_manager_kind()` | Closed for install: explicit service flags are validation overrides, while omitted flags use PAL-derived auto-detection. |
...
| `src/cli/wizard.rs` | 185 | Falls back to `ServiceChoice::None` only when platform detection itself fails. | `PAL-method` | `default_service_choice()` | Closed for unsupported hosts without pretending Linux defaults apply. |
| `src/cli/wizard.rs` | 245 | Interactive wizard obtains defaults from `WizardPlatformDefaults::detected()`. | `PAL-method` | `default_service_choice()` | Closed for interactive mode: prompts and defaults share the same PAL-derived default object. |
| `src/cli/wizard.rs` | 277 | `run_interactive_for_platform()` accepts an injected platform for host-independent service-default tests. | `PAL-method` | `default_service_choice()` | Closed for testability: callers can exercise Linux, macOS, and unsupported service choices without relying on runner OS cfg. |
| `src/cli/wizard.rs` | 638 | Unit test verifies auto mode maps mock Linux/systemd, macOS/launchd, and unsupported service kinds. | `PAL-method` | `default_service_choice()` | Closed by `auto_answers_detects_platform_service()`. |
| `src/cli/wizard.rs` | 709 | Interactive default test injects a launchd mock platform and accepts defaults. | `PAL-method` | `default_service_choice()` | Closed for macOS default coverage through `interactive_wizard_accepts_defaults()`. |
| `src/cli/wizard.rs` | 931 | Invalid service input falls back through an injected launchd mock platform default. | `PAL-method` | `default_service_choice()` | Closed by `interactive_wizard_service_invalid_falls_back()`. |
| `src/cli/wizard.rs` | 1064 | Unit test verifies `ServiceChoice::from_service_kind()` for systemd, launchd, and none. | `PAL-method` | `default_service_choice()` | Closed by `service_choice_maps_pal_service_kind()`. |
